...
export function IssueCertificateForm({ onCertificateIssued }: IssueCertificateFormProps) {
  const auth = useAuth();
  ...
  return (
     <Card>
        <CardHeader>
          ...
          <CardDescription>Fill in the details below to start the process.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField
                ...
                />
                <FormField
                  control={form.control} name="studentEmail"
                  render={({ field }) => (
                    <FormItem>
                ...
                />
                 <FormField
                  control={form.control} name="degreeName"
                  render={({ field }) => (
                    <FormItem>
                ...
                />
                 <FormField
                  control={form.control} name="degreeSubject"
                  render={({ field }) => (
                    <FormItem>
